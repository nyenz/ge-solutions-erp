#!/usr/bin/env python3
"""fix24.py — Ledger header polish + junk-seed cleanup + Folder page remodel.
Run: py fix24.py"""
import subprocess
...
# ---------------- Ledger: sticky header gap + opaque + mobile sticky ----------------
patch('erp-frontend/src/pages/Ledger/LedgerPage.jsx', "<CornerDecor />", "<CornerDecor hideTop />")
...
""".ledgerTable thead th{
    ...
    text-align:left;padding:clamp(11px,1.5vw,18px) clamp(12px,1.8vw,20px);
    border-bottom:3px solid var(--orange);white-space:nowrap;user-select:none;
    /* solid tan strip above the stuck header = breathing room + no see-through */
...
# ---------------- DataInitializer: robust purge + idempotent seed ----------------
...
subprocess.run(['git','commit','-m','fix24: ledger header spacing/opacity/mobile-sticky + top-corner decor off + junk-seed purge + folder remodel'],check=False,cwd=ROOT,capture_output=True)
