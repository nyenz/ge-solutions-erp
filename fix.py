#!/usr/bin/env python3
"""fix17.py — repair the two build-breakers, then re-commit.
1) FolderPage.jsx: de-duplicate the BackToTopButton import + usage (idempotent).
2) DataInitializer.java: FULL rewrite with a self-consistent seedOne (19 args
   everywhere) so javac passes.
3) LandController.java: FULL rewrite with one mapping per method (index fix).
Run: py fix17.py
"""
import subprocess
...
# ----------------------------------------------------------------------
# 1) FolderPage.jsx — de-duplicate BackToTopButton import + usage
# ----------------------------------------------------------------------
fp = ROOT / 'erp-frontend/src/pages/DigitalFolder/FolderPage.jsx'
t = fp.read_text(encoding='utf-8')
imp = "import BackToTopButton from '../../components/common/BackToTopButton';\n"
while t.count(imp) > 1:
    i1 = t.find(imp); i2 = t.find(imp, i1 + len(imp))
    t = t[:i2] + t[i2 + len(imp):]
use = "<BackToTopButton />"
while t.count(use) > 1:
    i1 = t.find(use); i2 = t.find(use, i1 + len(use))
    t = t[:i2] + t[i2 + len(use):]
if t.count(imp) == 0:
    t = t.replace("import styles from './FolderPage.module.css';\n",
                  "import styles from './FolderPage.module.css';\n" + imp, 1)
if t.count(use) == 0:
    t = t.replace("        </HardwareModal>\n    </div>\n);\n};\n\nexport default FolderPage;",
                  "        </HardwareModal>\n        <BackToTopButton />\n    </div>\n);\n};\n\nexport default FolderPage;", 1)
fp.write_text(t, encoding='utf-8')
print("OK: FolderPage.jsx BackToTopButton de-duplicated (import x%d, usage x%d)" % (t.count(imp), t.count(use)))

# ----------------------------------------------------------------------
# 2) DataInitializer.java — FULL rewrite, consistent seedOne signature
# ----------------------------------------------------------------------
(ROOT / 'erp-backend/src/main/java/com/gesolutions/erp/config/DataInitializer.java').write_text(r"""// PATH: erp-backend/src/main/java/com/gesolutions/erp/config/DataInitializer.java
...
""", encoding='utf-8')
print("OK: DataInitializer.java rewritten (consistent seedOne)")

# ----------------------------------------------------------------------
# 3) LandController.java — one mapping per method (index endpoint fix)
# ----------------------------------------------------------------------
lc = ROOT / 'erp-backend/src/main/java/com/gesolutions/erp/modules/land/controller/LandController.java'
t = lc.read_text(encoding='utf-8')
old = """    @PostMapping("/projects/{id}/unlock-log")
...
new = """    // FIX: one mapping per method (stacked annotations left /next-index unrouted)
    @PreAuthorize("hasAnyRole('ROLE_MANAGER', 'ROLE_SECRETARY', 'ROLE_ADMIN', 'ROLE_DIRECTOR')")
    @GetMapping("/next-index")
    public ResponseEntity<String> previewNextIndex() {
        return ResponseEntity.ok(landService.previewNextIndex());
...
if old in t:
    ...
    print("OK: LandController.java index endpoint split")
elif "@GetMapping(\"/next-index\")" in t and "logDossierUnlock" in t and t.index("@PostMapping(\"/projects/{id}/unlock-log\")") < t.index("@GetMapping(\"/next-index\")"):
    print("WARN: LandController still stacked but pattern differs -- manual check needed")
else:
    print("OK: LandController.java already clean")

# ----------------------------------------------------------------------
subprocess.run(['git', 'add', '.'], check=False, cwd=ROOT, capture_output=True)
subprocess.run(['git', 'commit', '-m', 'fix17: repair build (dedupe BackToTopButton import, consistent seedOne, split index mapping)'], check=False, cwd=ROOT, capture_output=True)
subprocess.run(['git', 'push'], check=False, cwd=ROOT, capture_output=True)
print("\nDone. Pushed. Watch Render for a GREEN build, then '>>> [SAMPLE] Seeded 10 detailed sample projects'.")