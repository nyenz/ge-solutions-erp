#!/usr/bin/env python3
"""fix30.py — add full scenario seed data to DataInitializer (idempotent).
Run: py fix30.py"""
import subprocess
from pathlib import Path
ROOT = Path(__file__).parent.resolve()

content = r"""// PATH: erp-backend/src/main/java/com/gesolutions/erp/config/DataInitializer.java
...
print("Wrote DataInitializer.java (scenario seed added)")

subprocess.run(['git','add','.'],check=False,cwd=ROOT,capture_output=True)
subprocess.run(['git','commit','-m','fix30: add full scenario seed data (all intake scenarios) to DataInitializer'],check=False,cwd=ROOT,capture_output=True)
