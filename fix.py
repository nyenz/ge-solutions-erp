# fix.py -- fix113: dedupe recoveryChips declaration (build fix) + re-apply folder batch (stage middle-insert, related projects own panel, docs add without edit)
import subprocess
...
BAK = ROOT / ".fix_backup" / "FolderPage.jsx.71"

# 0. BUILD FIX: remove the duplicated recoveryChips useState line (live file + backup)
for p in (JSX, BAK):
    if not p.exists():
        print("MISSING", p.name); continue
    s = read(p)
    needle = "const [recoveryChips, setRecoveryChips] = useState([]);"
    first = s.find(needle)
    if first < 0:
        print("MISSING recoveryChips line in", p.name); continue
    second = s.find(needle, first + len(needle))
    if second < 0:
        print("SKIP no duplicate in", p.name); continue
    start = s.rfind("\n", 0, second) + 1
    end = s.find("\n", second)
    if end < 0: end = len(s)
    write(p, s[:start] + s[end + 1:])
    print("OK removed duplicate recoveryChips line in", p.name)

# 0b. recovery chips show RECOVERY-sourced notes only
for p in (JSX, BAK):
    patch(p, "const all = lists.flat().sort((a, b) =>",
             "const all = lists.flat().filter(n => n.source === 'RECOVERY').sort((a, b) =>",
             "chips filter RECOVERY only in " + p.name)

# 1. icons for plus button + related projects panel
patch(JSX,
...
# 3. openAddModal accepts a position
patch(JSX,
...
# 4. attach then reorder so the new stage lands at the chosen position
patch(JSX,
"""setSaving(true);
...
# 7. related projects drawer state
patch(JSX,
...
# 9. notes wrapper gets print-safe class
patch(JSX,
"""<div style={activeTab !== 'NOTES' ? { display: 'none' } : {}}>""",
"""<div className={styles.tabWrap} style={activeTab !== 'NOTES' ? { display: 'none' } : {}}>""",
"notes tabWrap class")

# 10. owners tab wrapper opens (closed again after the related panel)
patch(JSX,
...
# 11. remove the per-owner OTHER PROJECTS block (moves to its own panel)
...
# 12. RELATED PROJECTS becomes its own panel; closes the owners tabWrap div
...
# 13 + 14. documents: add without edit mode, delete unchanged (edit only)
patch(JSX,
...
if "fix113" not in s:
    s += """
/* fix113: middle-insert plus button + insert position line + print-safe tab wrapper */
...
"""
    write(cssp, s); print("OK fix113 css")
else:
    print("SKIP fix113 css already present")

try:
    subprocess.run(["git", "add", "-A"], cwd=ROOT, check=True)
    subprocess.run(["git", "commit", "-m", "fix113: dedupe recoveryChips declaration, re-apply folder stage middle-insert + related panel + docs add-without-edit"], cwd=ROOT, check=True)
