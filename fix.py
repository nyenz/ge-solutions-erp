...
# ══════════════════════════════════════════════════════════════════
# FIX: Unified filter button style — Payments + Ledger pages
#
# PROBLEM:
#   PaymentsPage filter buttons: background rgba(255,255,255,0.08)
#   = near-transparent on cream background = white text on white = unreadable
#
# SOLUTION:
#   Match Ledger's dark inactive style on both pages.
#   Unified standard:
#     Inactive:  dark navy bg, white text, subtle border
#     Hover:     orange border + orange text, slight orange bg tint
#     Active:    solid orange bg, dark navy text
# ══════════════════════════════════════════════════════════════════
...
def patch(path, old, new, label):
    ...
    if old not in content:
        print(f"  MISSING (already patched or text mismatch): {label}")
        return
...
print("=== PATCHING PaymentsPage.module.css ===")

PAYMENTS_CSS = "erp-frontend/src/pages/Payments/PaymentsPage.module.css"

# Replace the inactive filterBtn block — the near-transparent white bg is the bug
patch(
    PAYMENTS_CSS,
    """.filterBtn {
...
.filterBtn:hover { border-color: #EE8C3A; color: #EE8C3A; background: rgba(238,140,58,0.12); }""",
    "Payments filterBtn inactive — dark navy bg, readable white text"
)


print("\n=== PATCHING LedgerPage.module.css ===")

LEDGER_CSS = "erp-frontend/src/pages/Ledger/LedgerPage.module.css"

# Ledger inactive is already correct (dark navy bg) — just align font-weight to 900
# and make hover consistent with Payments
patch(
    LEDGER_CSS,
    """.filterBtn {
    flex: 0 0 auto;
...
.activeFilter {
    ...
    """.filterBtn {
    flex: 0 0 auto;
    /* Inactive: dark navy bg, clearly readable white text */
    background: rgba(26, 46, 48, 0.75);
    border: 1.5px solid rgba(255, 255, 255, 0.18);
    color: rgba(255, 255, 255, 0.85);
    padding: clamp(8px, 1.1vw, 12px) clamp(12px, 1.6vw, 22px);
...
/* Hover: orange border + orange text, slight tint — always readable */
.filterBtn:hover { background: rgba(238, 140, 58, 0.12); color: #EE8C3A; border-color: var(--orange); }
.filterBtn:focus-visible { outline: 2px solid var(--orange); outline-offset: 2px; }
...
.activeFilter {
    ...
    "Ledger filterBtn — comment update + confirm hover stays readable"
)
...
print("\nAll done.")
print("If both patches show OK, commit and push.")
print("If MISSING is shown, the CSS already has that exact style applied.")